...
def ajax_url(request):
    array_value = request.POST.getlist('traitvalues[]')
    value = request.POST.get('traitvalues')
    value2 = request.POST.get('traitvalues[]')
    dictionary = request.POST.get('traitvalues{}')
    print("ajax: ")
    print(str(array_value))
    print(str(value))
    print(str(value2))
...
def parse(request, pk):
    ...
    if request.method == 'POST':
        ...
        arr = json.loads(request.GET.get['traitvalues'])
        arr2 = json.loads(request.GET.get['traitvalues[]'])
        traitvalues =  request.GET.get('traitvalues[]')
        traitvalues2 = request.GET.get('traitvalues')
        # arr = request.POST.get('traitvalues')
        print("arr: ", str(arr))
        print("arr2: ", str(arr2))
        print("arr3: ", str(traitvalues))
        print("arr4: ", str(traitvalues2))
