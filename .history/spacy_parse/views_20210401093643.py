...
# create a serializer
class SentsSerializer(serializers.Serializer):
    # intialize fields
    strings = serializers.ListField(
    child = serializers.StringField(min_value = 0, max_value = 100)
    )
...
def parse(request, pk):
    ...
    if request.method == 'POST':
        ...
        # print('data', data)
        # print(type(sentences_with_traits))
        # json_sentences = serializers.serialize('json', sentences_with_traits, cls=LazyEncoder)
        # print(type(json_sentences))
        # json_sents = json.dumps(json_sentences)
        # print(type(json_sents))
        serializer = ListSerializer(sentences_with_traits)
