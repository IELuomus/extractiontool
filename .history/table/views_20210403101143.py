...
def json_table_list(request, user_id, pdf_id):
    json_tables= Json_Table.objects.filter(pdf_id=pdf_id).values_list('id', flat=True)
    print(json_tables)
...
def table_to_dataframe(request):
    ...
    if camelot_tables:
        ...
        return render(request, 'selecte_tables.html', {'object_list': products})
        return HttpResponseRedirect(reverse('selected_tables', args=(user_id, pdf_id )))
        # return "%s?" % (redirect('selected_tables', args=(user_id, pdf_id )))
