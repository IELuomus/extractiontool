...
def json_table_list(request, user_id, pdf_id, page_number):
    ...
    i = 1
    for input in html_tables:
        text_file = open("templates/table" + str(i) +
                            ".html", "w", encoding='utf-8')
        text_file.write(input)
        i = i+1
        jobs = len(html_tables)

    return render(request, 'selected_tables.html', {'html_tables': html_tables, 'object_list': json_tables, 'table_list': table_list, 'jobs': str(jobs) })
...
def table_to_dataframe(request):
    ...
    for table in tables:
        ...
        table.to_json('media/json/' + file_name + '-page-' + page_number +
                          '-table-' + str(i) + '.json', orient='records', index=False)
