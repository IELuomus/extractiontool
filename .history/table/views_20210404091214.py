...
def json_table_list(request, user_id, pdf_id, page_number):
    ...
    for table in json_tables:
        ...
        # dict_train = table.json_table
        #
        # train = pd.read_json(dict_train, orient='table')
        # train.reset_index(level=0, inplace=True)
        input = json2html.convert(json = table.json_table)
        # table = train.to_html()
        # text_file = open("templates/table" + str(i) +
        #                  ".html", "w", encoding='utf-8')
        # text_file.write(table)
        print(input)
        html_tables.append(input)
        i= i+1


    jobs = str(num_tables[0])
    print('jobs: ', jobs)

    return render(request, 'selected_tables.html', {'html_tables': html_tables, ' json_tables': json_tables, 'table_list': table_list, 'jobs': jobs })
...
def table_to_dataframe(request):
    ...
    for table in tables:
        ...
        table.to_json('media/json/' + file_name + '-page-' + page_number +
                          '-table-' + str(i) + '.json', orient='table', index=False, force_ascii=False)

        ROOT_FILE = 'media/json/' + file_name + '-page-' + \
            page_number + '-table-' + str(i) + '.json'
        json_data = open(ROOT_FILE, encoding='utf-8')
