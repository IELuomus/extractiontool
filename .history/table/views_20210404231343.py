...
from html.entities  import HTML
...
def json_table_list(request, user_id, pdf_id, page_number):
    ...
    for entry in file_paths:
        ...
        text_file = open("table" + str(i)+".html", "w")
        html_dataframes2.append(HTML(data.to_html(classes='table table-striped')))
    ...
    return render(request, 'selected_tables.html', {'dataframes':data_frames, 'html_dataframes': html_dataframes, 'html_dataframes2': html_dataframes2})
...
def table_to_dataframe(request):
    ...
    if camelot_tables:
        ...
        #    'media/json/PURS: Personalized Unexpected Recommender System for Improving User Satisfaction_camelot-page-8-table-3.json'
